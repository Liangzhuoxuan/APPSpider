...
class BiKeErSpider(ParseComponent):
    def __init__(self, keyword):
        super(BiKeErSpider, self).__init__("", keyword)
        self.url = "http://www.bkill.com/search/?wd={}".format(self.quote_keyword)
        # self.n_page = 2
        self.name = "贝克尔下载"
    ...
    def get_app_list_elements(self, url) -> list:
        ...
        lis = selector.xpath('//div[@class="result"]//li')
        return lis
    ...
    def get_app_name(self, li):
        ...
        app_name = li.xpath("div[1]/a/text()")
        app_name = self.judge_null(app_name)
    ...
    def get_download_url(self, inner_response):
        ...
        # download_url = selector.xpath('//ul[@class="downandroid"]/li[1]/a/@href')
        # download_url = self.judge_null(download_url)
        # download_url = "https:" + download_url
        return None

    def get_enter_url(self, li):
        enter_url = li.xpath("div[1]/a/@href")
    ...
    def get_img_address(self, li):
        img_address = li.xpath("div/a/img/@src")
    ...
    def get_app_intro(self, inner_response):
        # selector = etree.HTML(inner_response)
        app_intro = self.li.xpath("div[2]/p[1]/text()")
...
if __name__ == '__main__':
    lvse_spider = BiKeErSpider(keyword="支付")
    lvse_spider.parse()
