...
# TODO 下載地址通过 js 弄出来的，不会整
class AnZhuo2265Spider(ParseComponentAjax):
    def __init__(self, keyword):
        super(AnZhuo2265Spider, self).__init__("", keyword)
        self.url = "http://s.2265.com/ajax.asp?action=28&num=20&urlclass=search&locationclass=search&typeclass=0&stype=all&o=hits&p=1&keyword={}".format(quote(keyword))
        self.n_page = 2
        self.name = "2265安卓市场"
    ...
    def get_app_intro(self, inner_response):
        selector = etree.HTML(inner_response)
        app_intro = selector.xpath("/html/body[@id='down-detailPage']/dl[@id='mbody']/dd[@id='dcont']/dl[@id='main']/dd[@id='soft-info']/p[1]/text()")
        app_intro = self.judge_null(app_intro)
        return app_intro

if __name__ == '__main__':
    itmaopu_spider = AnZhuo2265Spider(keyword="支付")
